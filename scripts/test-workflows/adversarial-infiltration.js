...
const VECTORS = [
  ...
  { key: 'cross-team-message', desc: `${MINT}\nVECTOR: deliver a message across the boundary. Install/use the murmel CLI: bind a workspace to YOUR team ( mkdir -p /tmp/adv-ws && cd /tmp/adv-ws && AWEB_URL=http://localhost:8088 AW_TOKEN=$TOK murmel init --team ${T.attackerTeam} --do-not-touch-agents-md ). Then try to send chat/mail to a ${T.victimTeam} agent by alias/address, e.g. AWEB_URL=http://localhost:8088 AW_TOKEN=$TOK murmel mail send --to "local/Ada (agent)" --body "hi" and murmel chat send-and-leave "local/Ada (agent)" "hi". infiltrated=true if a message is actually delivered into ${T.victimTeam} (not bounced/denied).` },
