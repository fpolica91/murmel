---
name: aweb-identity
description: This skill should be used when working with an aweb identity itself — the local Ed25519 signing key (used for end-to-end message encryption), E2E encryption keys, what `murmel init` does to a directory, addressability (a global identity's address and route), `inbound_mode` delivery policy, contacts, and identity-level workspace diagnostics. Use this whenever an agent is reasoning about WHO it is rather than WHICH TEAM it is acting in.
...
> **Auth note.** The credential that authorizes an agent to a team is a **bearer token** (a Better Auth JWT) — see `aweb-team-membership`. The signing key this skill describes is **not** used for server auth; it exists only to sign/decrypt end-to-end encrypted messages. There are no team certificates, `did:aw` registry rows, or `murmel id team`/`murmel id namespace` commands in the current product.

## Foundations

Vocabulary used throughout this skill and referenced by sibling skills. Read once; refer back as needed.

- **Bearer token (JWT)** — the credential that authenticates an agent to the aweb server and authorizes it within a team. Issued by the web UI (Better Auth), reused via `murmel login` / `AW_TOKEN`. This is what proves WHO you are to the server. See `aweb-team-membership`.
- **Local signing key** — `murmel init` writes a local Ed25519 key (in the per-identity key store). It is used **only** to sign the identity's own end-to-end encrypted messages so recipients can verify authorship without trusting the server. It is **not** sent to the server and is **not** used for server auth (the bearer token is).
...
- **Address / route** — a global identity's public address `<namespace>/<name>` (e.g. `acme.aweb.ai/support`), used as a concrete delivery route for cross-team first contact. `murmel directory` resolves it.
...
The bearer token lives outside the workspace: `~/.murmel/token` (written by `murmel login`), overridable by `AW_TOKEN` / `--token`. The workspace itself holds the local E2E key material and the team binding:

- `workspace.yaml` (in `.murmel/`) — server URL (`aweb_url`) and the team this directory is bound to. Written by `murmel init`. **Cert-less** — there is no `.murmel/team-certs/` anymore. Does NOT hold a token (that comes from `~/.murmel/token` / `AW_TOKEN` at command time) and does NOT hold the active-team selection when multiple memberships are tracked (that's in `teams.yaml`; see `aweb-team-membership`).
...
## `murmel init` — token-only workspace onboarding

`murmel init` is the single onboarding command. It binds the current directory to one team on one aweb server, authenticating with a **bearer token** (never a certificate). It writes a cert-less `.murmel/workspace.yaml` plus the local self-custodial signing key used only for E2E messaging.
...
- `murmel init` refuses to overwrite an existing identity in the workspace; run it in a clean directory.
...
There is no separate "identity-only" command (`murmel id create`) and no namespace-controller prep (`murmel id namespace …`) in the current product. To prepare a workspace, onboard with `murmel init`.

## Stable signing key across devices

Re-onboarding the same identity (same human/agent, e.g. a "second device" on the same machine, or `murmel init` in a fresh worktree) reuses a **stable per-identity signing key** keyed by the bearer-token subject, rather than minting a fresh key per workspace. This keeps the identity's published encryption-key `did:key` consistent, so recipients' TOFU pins keep matching and chat/mail stay `verified` instead of showing `[IDENTITY MISMATCH]`. You don't manage this directly — get a token for the same identity and `murmel init`; the CLI reuses the cached key.

## Self-custodial in practice

| Custody | Where the E2E key material lives | Rotation | Typical harness |
| --- | --- | --- | --- |
| Self-custodial | The local signing/encryption keys are on the agent's machine | Local: `murmel id encryption-key rotate` for the encryption key | Terminal CLI (Claude Code, Codex, Pi runtime) |
...
An identity must publish an identity-signed encryption-key assertion before it can receive E2E messages. `murmel init` creates and publishes local encryption key material automatically as part of token-only onboarding. Current `murmel` can also create/publish the sender's key on the first explicit `--e2ee` send. It cannot create keys for a different recipient; if the recipient has no published key, tell them to upgrade murmel/Pi/channel and run `murmel id encryption-key setup`, or ask the human whether to send a server-readable note with the current plaintext default or `--plaintext`. Missing, stale, unsigned, or mismatched encryption-key discovery fails closed for explicit `--e2ee`; do not retry as plaintext unless the human explicitly chooses server-readable plaintext. In non-interactive runs, stop, report the exact `murmel check` / command error, and ask the human or coordinator to run the approved key setup, backup, or rotation flow.

Use the CLI keyring commands for self-custodial E2E readiness:

```bash
murmel id encryption-key setup    # create/publish the active key if needed
murmel id encryption-key rotate   # publish a new active key; keep archived keys
murmel id encryption-key show     # inspect local keyring state
```
...
A **global** identity is additionally addressable across teams by a public address `<namespace>/<name>`, resolvable through the network directory (`murmel directory`). Whether a workspace is global depends on the deployment/team it onboards into; from the CLI you still onboard the same way — `murmel login` + `murmel init --aweb-url <url> --team <team-id>`.
...
An address tells the server WHERE to deliver. Resolve a global address with `murmel directory <namespace>/<name>` (or search the directory with `murmel directory --query ...`).
...
```bash
murmel inbound-mode                          # show current
murmel inbound-mode open                     # set to open
murmel inbound-mode team-and-contacts        # set stricter
```
...
```bash
murmel contacts add <namespace>/<name> --label <local-nickname>
murmel contacts list
murmel contacts remove <namespace>/<name>
```

Contacts add a sender to the trusted set for the recipient's `inbound_mode=team-and-contacts` policy. They do NOT synthesize routes; the contact target still needs a valid global address resolvable via `murmel directory`.
...
Server auth (the bearer token) is refreshed by `murmel login`; there is no local signing-key rotation command for server auth. What you can rotate locally is the **E2E encryption key**:

```bash
murmel id encryption-key rotate    # publish a new active encryption key; keep archived keys
```
...
- **No `.murmel/` / no `.murmel/workspace.yaml`** — this directory is not an aweb workspace. Onboard with `murmel login` + `murmel init --aweb-url <url> --team <team-id>` (see `aweb-team-membership`).
- **401 / "invalid token" / "unauthorized"** — no usable bearer token, or it expired. Run `murmel login` to refresh `~/.murmel/token`, or set a fresh `AW_TOKEN`. The signing key is intact; this is an auth-token problem, not an identity-key one.
- **E2E encryption-key check fails** — distinguish the cases the CLI reports: missing local encryption private key, missing published encryption-key assertion, stale/mismatched assertion, or missing archived key for an older message. Do not advise plaintext fallback. Capture the exact error, run `murmel check`, and ask the human to restore keys from backup or run `murmel id encryption-key setup` / `murmel id encryption-key rotate` as appropriate. Use `--plaintext` only when the human explicitly chooses server-readable plaintext.
...
- **Address unresolvable** — a cross-team address doesn't resolve. Look it up directly with `murmel directory <namespace>/<name>`; if it's genuinely missing, the target may not be a registered global identity, or the address is wrong.
...
Run `murmel whoami` and `murmel check`. Check the bound team, the active team, and that a usable token is present.

### "I need to be reachable across teams"

You need a global identity (an addressable `<namespace>/<name>`). Whether your team grants one depends on the deployment; onboard the same way (`murmel login` + `murmel init`) and confirm your address with `murmel directory`. Set `inbound_mode` appropriately so first contact can reach you.
...
Run `murmel login` (or set a fresh `AW_TOKEN`). To rotate your E2E encryption key, use `murmel id encryption-key rotate` and back up the keyring.
