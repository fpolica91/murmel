...
description: This skill should be used when onboarding an agent into an aweb team, obtaining and using the bearer token (Better Auth JWT) that authorizes coordination, binding a directory to a team with `aw init`, selecting the active team across multiple memberships, or diagnosing token/membership/active-team failures. Use this whenever the question is about WHICH TEAM the agent acts in or how it became a member.
...
> **Auth in one sentence.** The only credential aweb accepts is a **bearer token** — a Better Auth JWT. A human gets one by signing up / logging in to the web UI; an agent reuses that token via `aw login` (cached at `~/.aw/token`) or the `AW_TOKEN` env var. **Membership in a team is what grants access** to that team's coordination state. There are no team certificates, no DIDs, no `aw id team`/`aw id namespace` steps, and no BYOT controller keys in the current product.
...
- **Team id** — canonical form is `<name>:<namespace>` (e.g. `default:local`, `acme:acme.aweb.ai`). Pass it to `aw init --team <team-id>` and to `--team <team-id>` overrides.
...
- **Workspace binding** — `.aw/workspace.yaml` ties the current directory to one aweb server (`aweb_url`) and one team. `aw init` writes it; it is **cert-less** (no `.aw/team-certs/`).
- **Self-custodial signing key** — `aw init` also writes a local Ed25519 signing key used **only** for end-to-end message encryption (see `aweb-identity`). It is never used to authenticate to the server — that is always the bearer token.

## Token-related files in `.aw/` and `~/.aw/`

- `~/.aw/token` — the cached bearer token (and its refresh token) written by `aw login`. Reused and auto-refreshed by later commands. `AW_TOKEN` (or `--token <jwt>`) overrides it for non-interactive use.
- `.aw/workspace.yaml` — server URL + the team this directory is bound to. Written by `aw init`. Does not hold a token; the token comes from `~/.aw/token` / `AW_TOKEN` at command time.
- `.aw/teams.yaml` — local index of teams known to this workspace, with `active_team:` selecting the default for commands run here. Present when the workspace tracks more than one membership.
...
To use that token from the CLI, either run `aw login` (browser device flow, caches the token) or copy the raw JWT and export it as `AW_TOKEN`.
...
aw login                                   # interactive: browser device-auth, caches ~/.aw/token
...
# 3. Verify.
aw check          # identity, workspace, team, connectivity
aw whoami
aw workspace status
```
...
- `aw init` refuses to overwrite an existing identity in `.aw/`; run it in a clean directory (or one not yet bound).
...
- `aw init` updates the `aweb` section of `AGENTS.md` / `CLAUDE.md` by default; pass `--do-not-touch-agents-md` to skip.
...
- **No `.aw/workspace.yaml`** — this directory is not bound to any team/server. Onboard with `aw login` + `aw init --aweb-url <url> --team <team-id>`.
- **401 / "invalid token" / "unauthorized"** — no usable bearer token, or it expired. Run `aw login` to refresh `~/.aw/token`, or set a fresh `AW_TOKEN`. If the stack is up and the token is fresh, the issuer/audience/JWKS config may be mismatched on the server side (a deployment problem, not a workspace one).
...
One token subject can be a member of several teams. A workspace can track multiple memberships in `.aw/teams.yaml`; `active_team:` selects which team coordination commands reach by default. Override per command with `--team <team-id>`.

```bash
aw workspace status                          # shows the active team
aw <verb> --team <team-id> ...               # one-off override for team-scoped commands
```

To make a different team the default for a directory, bind that directory to it with `aw init --team <team-id>` (in a fresh directory or worktree), or set `active_team:` if the workspace already tracks both memberships. Acting in the wrong active team can send messages, claims, or locks to the wrong coordination boundary — confirm the active team before relying on local aliases or claiming work.
...
Treat teams as separate coordination boundaries for tasks, locks, roles, instructions, presence, and same-team aliases. Cross-team mail/chat first contact uses an explicit address route (`<namespace>/<alias>`); continuations reuse the recorded route. Confirm `active_team:` (via `aw workspace status`) before relying on local aliases, claiming work, or choosing sender context — or use `--team <team-id>` for a one-off override.

### "Coordination commands are unauthorized (401/403)"

1. `aw whoami` / `aw check` — is there a usable token at all? If not, `aw login` (or set `AW_TOKEN`).
2. If the token is fresh but you still get 403, the subject isn't a member of the target team — add it through the UI.
3. Confirm you are addressing the team you're actually a member of: `aw workspace status` shows the active team; use `--team <team-id>` to target another.

### "I onboarded but commands hit the wrong team"

The workspace tracks more than one membership and `active_team:` doesn't match what you want. Pass `--team <team-id>` to specific commands, or re-bind the directory with `aw init --team <team-id>` in a clean worktree for the team you want as the default.
...
- `ai-completion/GETTING-STARTED.md` (aweb repo checkout): the canonical end-to-end token-only onboarding walkthrough (human sign-up + agent `aw init`).
